...
class Arrays
{
     ...
     * $output = \Ws\Helper\Arrays::normalize($input);
     * // $output 现在是一个数组，结果如下：
     * // $output = [
     * //   'item1',
     * //   'item2',
     ...
     * @code php
     * $arr = ['', 'test', '   '];
     * \Ws\Helper\Arrays::remove_empty($arr);
     *
     * \Ws\Env::dump($arr);
     *   // 输出结果中将只有 'test'
     * @endcode
     ...
     *     ['id' => 2, 'value' => '2-1'],
     * ];
     * $values = \Ws\Helper\Arrays::cols($rows, 'value');
     *
     * \Ws\Env::dump($values);
     *   // 输出结果为
     *   // [
     ...
     *     ['id' => 2, 'value' => '2-1'],
     * ];
     * $hashmap = \Ws\Helper\Arrays::hashmap($rows, 'id', 'value');
     *
     * \Ws\Env::dump($hashmap);
     *   // 输出结果为
     *   // [
     ...
     *     ['id' => 2, 'value' => '2-1'],
     * ];
     * $hashmap = \Ws\Helper\Arrays::hashmap($rows, 'id');
     *
     * \Ws\Env::dump($hashmap);
     *   // 输出结果为
     *   // [
     ...
     * ];
     * $values = \Ws\Helper\Arrays::group_by($rows, 'parent');
     *
     * \Ws\Env::dump($values);
     *   // 按照 parent 分组的输出结果为
     *   // array(
     ...
     * ];
     *
     * $tree = \Ws\Helper\Arrays::tree($rows, 'id', 'parent', 'nodes');
     *
     * \Ws\Env::dump($tree);
     *   // 输出结果为：
     *   // [
     ...
     * @code php
     * $refs = null;
     * $tree = \Ws\Helper\Arrays::tree($rows, 'id', 'parent', 'nodes', $refs);
     * 
     ...
     * ];
     *
     * $rows = \Ws\Helper\Arrays::sort_by_col($rows, 'id', SORT_DESC);
     * \Ws\Env::dump($rows);
     * // 输出结果为：
     * // [
     * //   ['id' => 6, 'value' => '6-1', 'parent' => 3],
     ...
     * 用法：
     * @code php
     * $rows = \Ws\Helper\Arrays::sort_by_cols($rows, [
     *     'parent' => SORT_ASC, 
     *     'name' => SORT_DESC,
     * ]);
     * @endcode
